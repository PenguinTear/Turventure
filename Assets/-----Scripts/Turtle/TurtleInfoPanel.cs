...
namespace LittleTurtle.UI {
    public class TurtleInfoPanel : MonoBehaviour{
        ...
        //
        public TurtleCtrl turtleCtrl;  // current turtle
        ...
        public void OnHPChange() {
            float f = turtleCtrl.HP / turtleCtrl.data_turtle.MaxHP;
            HP_percent.text = (f * 100).ToString("0") + "%";
            HP_Value.text = turtleCtrl.HP.ToString() + "/" + turtleCtrl.data_turtle.MaxHP;
            _lerping_HealthBar.SetTarget(f, CurveLerping.CurveType.EaseOut_Slow);
        }
        private void OnGrowthChange() {
            float f = turtleCtrl.Growth / turtleCtrl.data_turtle.MaxGrowth;
            Growth_percent.text = (f * 100).ToString("0") + "%";
            Growth_Value.text = turtleCtrl.Growth.ToString() + "/" + turtleCtrl.data_turtle.MaxGrowth;

            _lerping_GrowthBar.SetTarget(f, CurveLerping.CurveType.EaseOut_Slow);
        }


        public void InitTurtleData() {
            _StateSwitch((int)turtleCtrl.State);
            OnHPChange();
            OnGrowthChange();
        }
